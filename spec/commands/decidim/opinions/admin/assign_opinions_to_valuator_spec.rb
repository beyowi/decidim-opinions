...
module Decidim
  module Opinions
    module Admin
      describe AssignOpinionsToValuator do
        describe "call" do
          ...
          describe "when the form is valid" do
            ...
            it "creates the valuation assignment between the user and the opinion" do
              expect do
                command.call
              end.to change { ValuationAssignment.where(opinion: opinion, valuator_role: valuator_role).count }.by(1)
            end

            it "traces the action", versioning: true do
              expect(Decidim.traceability)
                .to receive(:create!)
                .with(Decidim::Opinions::ValuationAssignment, form.current_user, opinion: opinion, valuator_role: valuator_role)
                .and_call_original
            ...
            context "when it raises an error while creating assignments" do
              before do
                allow(Decidim::Opinions::ValuationAssignment).to receive(:create!).and_raise(ActiveRecord::RecordInvalid)
              end
