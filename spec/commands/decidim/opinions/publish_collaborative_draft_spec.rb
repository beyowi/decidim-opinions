      ...
      describe "call" do
        ...
        context "when everything is ok" do
          ...
          it "creates a new opinion" do
            expect { command.call }
              .to change(Decidim::Opinions::Opinion, :count)
              .by(1)
          end
          ...
          it "transfers the attributes correctly" do
            ...
            expect(opinion.category).to eq(collaborative_draft.category)
            expect(opinion.scope).to eq(collaborative_draft.scope)
            expect(opinion.address).to eq(collaborative_draft.address)
            expect(opinion.attachments).to eq(collaborative_draft.attachments)
          end
        end
      end
    end
