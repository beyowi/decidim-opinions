...
module Decidim
  module Opinions
    describe MarkdownToOpinions do
      def should_parse_and_produce_opinions(num_opinions)
        opinions = Decidim::Opinions::Opinion.where(component: component)
        expect { parser.parse(document) }.to change { opinions.count }.by(num_opinions)
        opinions
      end

      def should_have_expected_states(opinion)
        expect(opinion.draft?).to be true
        expect(opinion.official?).to be true
      end

      def opinion_should_conform(section_level, title, body)
        opinion = Decidim::Opinions::Opinion.where(component: component).last
        expect(opinion.participatory_text_level).to eq(Decidim::Opinions::ParticipatoryTextSection::LEVELS[section_level])
        expect(opinion.title).to eq(title)
        expect(opinion.body).to eq(body)
      end

      let!(:component) { create(:opinion_component) }
      let(:parser) { MarkdownToOpinions.new(component, create(:user)) }
      let(:items) { [] }
      let(:document) do
        items.join("\n")
      end
      ...
      describe "titles create sections and sub-sections" do
        context "with titles of level 1" do
          ...
          it "create sections" do
            should_parse_and_produce_opinions(1)

            opinion = Opinion.last
            expect(opinion.title).to eq(title)
            expect(opinion.body).to eq(title)
            expect(opinion.position).to eq(1)
            expect(opinion.participatory_text_level).to eq(ParticipatoryTextSection::LEVELS[:section])
            should_have_expected_states(opinion)
          end
        end
        ...
        context "with titles of deeper levels" do
          ...
          it "create sub-sections" do
            ...
            opinions = should_parse_and_produce_opinions(5)

            opinions.order(:position).each_with_index do |opinion, idx|
              expect(opinion.title).to eq(titles[idx])
              expect(opinion.body).to eq(titles[idx])
              expect(opinion.position).to eq(expected_pos)
              expected_pos += 1
              expect(opinion.participatory_text_level).to eq("sub-section")
              should_have_expected_states(opinion)
            end
          end
        end
      ...
      describe "paragraphs create articles" do
        ...
        it "produces a opinion like an article" do
          should_parse_and_produce_opinions(1)

          opinion = Opinion.last
          # opinion titled with its numbering (position)
          expect(opinion.title).to eq("1")
          expect(opinion.body).to eq(paragraph)
          expect(opinion.position).to eq(1)
          expect(opinion.participatory_text_level).to eq(ParticipatoryTextSection::LEVELS[:article])
          should_have_expected_states(opinion)
        end
      end
      ...
      describe "links are parsed" do
        ...
        it "contains the link as an html anchor" do
          ...
          opinion = Opinion.last
          # opinion titled with its numbering (position)
          # the paragraph and opinion's body
          expect(opinion.title).to eq("1")
      ...
      describe "formats are parsed" do
        ...
        it "transforms formated texts to html tags" do
          ...
          paragraph = <<~EOEXPECTED
            ...
            - &lt;strong>double underscores&lt;/strong>
          EOEXPECTED
          expect(opinion.body).to eq(paragraph.strip)
          expect(opinion.position).to eq(1)
          expect(opinion.participatory_text_level).to eq(ParticipatoryTextSection::LEVELS[:article])
          should_have_expected_states(opinion)
        end
      ...
      describe "lists as a whole" do
        context "when unordered" do
          ...
          it "are articles" do
            should_parse_and_produce_opinions(1)
            opinion_should_conform(:article, "1", list)
          end
        end
