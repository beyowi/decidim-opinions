...
module Decidim
  module Opinions
    describe OpinionVotesHelper do
      ...
      before do
        ...
        allow(helper).to receive(:current_component).and_return(opinion_component)
      ...
      describe "#vote_button_classes" do
        it "returns small buttons classes from opinions list" do
          expect(helper.vote_button_classes(true)).to eq("card__button")
        end

        it "returns expanded buttons classes if it's not from opinions list'" do
          expect(helper.vote_button_classes(false)).to eq("expanded")
        end
      end

      describe "#votes_count_classes" do
        it "returns small count classes from opinions list" do
          expect(helper.votes_count_classes(true)).to eq(number: "card__support__number", label: "")
        end

        it "returns expanded count classes if it's not from opinions list'" do
      ...
      describe "#remaining_votes_count_for" do
        it "returns the remaining votes for a user based on the component votes limit" do
          opinion = create(:opinion, component: opinion_component)
          create(:opinion_vote, author: user, opinion: opinion)
