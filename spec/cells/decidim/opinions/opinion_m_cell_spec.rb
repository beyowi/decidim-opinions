...
module Decidim::Opinions
  describe OpinionMCell, type: :cell do
    controller Decidim::Opinions::OpinionsController

    subject { cell_html }

    let(:my_cell) { cell("decidim/opinions/opinion_m", opinion, context: { show_space: show_space }) }
    let(:cell_html) { my_cell.call }
    let(:created_at) { Time.current - 1.month }
    let(:published_at) { Time.current }
    let!(:opinion) { create(:opinion, created_at: created_at, published_at: published_at) }
    let(:model) { opinion }
    let(:user) { create :user, organization: opinion.participatory_space.organization }
    let!(:emendation) { create(:opinion) }
    ...
    context "when rendering" do
      ...
      context "and is a opinion" do
        it "renders the opinion state (nil by default)" do
      ...
      context "and is an emendation" do
        ...
        let(:my_cell) { cell("decidim/opinions/opinion_m", emendation, context: { show_space: show_space }) }
        let(:cell_html) { my_cell.call }
      ...
      context "when it is a opinion preview" do
        subject { cell_html }

        let(:my_cell) { cell("decidim/opinions/opinion_m", model, preview: true) }
        let(:cell_html) { my_cell.call }
