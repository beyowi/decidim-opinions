...
describe Decidim::Opinions::Metrics::AcceptedOpinionsMetricManage do
  let(:organization) { create(:organization) }
  let(:participatory_space) { create(:participatory_process, :with_steps, organization: organization) }
  let(:component) { create(:opinion_component, :published, participatory_space: participatory_space) }
  let(:day) { Time.zone.yesterday }
  let!(:accepted_opinions) { create_list(:opinion, 3, :accepted, created_at: day, component: component) }
  let!(:not_accepted_opinions) { create_list(:opinion, 3, created_at: day, component: component) }
  ...
  context "when executing" do
    ...
    it "updates metric records" do
      create(:metric, metric_type: "accepted_opinions", day: day, cumulative: 1, quantity: 1, organization: organization, category: nil, participatory_space: participatory_space)
      registry = generate_metric_registry
