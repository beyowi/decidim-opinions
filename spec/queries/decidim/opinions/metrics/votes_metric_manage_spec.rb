...
describe Decidim::Opinions::Metrics::VotesMetricManage do
  ...
  let!(:votes) { create_list(:opinion_vote, 5, opinion: opinion, created_at: day) }
  ...
  context "when executing" do
    ...
    it "updates metric records" do
      create(:metric, metric_type: "votes", day: day, cumulative: 1, quantity: 1, organization: organization, category: nil, participatory_space: participatory_space, related_object: opinion)
      registry = generate_metric_registry
    ...
    context "when calculating the metrics" do
      let(:withdrawn_opinion) { create(:opinion, state: "withdrawn", component: component) }
      let!(:invalid_votes) { create_list(:opinion_vote, 5, opinion: withdrawn_opinion, created_at: day) }
      let(:moderation) { create(:moderation, reportable: opinion, report_count: 1, participatory_space: participatory_space) }
      let!(:report) { create(:report, moderation: moderation) }

      it "filters the data correctly" do
        opinion.moderation.update!(hidden_at: Time.current)
