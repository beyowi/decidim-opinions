...
describe Decidim::Opinions::Metrics::EndorsementsMetricManage do
  ...
  let!(:endorsements) do
    5.times.collect do
      create(:endorsement, resource: opinion, created_at: day, author: build(:user, organization: opinion.organization))
    end
  end
  let!(:old_endorsements) do
    5.times.collect do
      create(:endorsement, resource: opinion, created_at: (day - 1.week), author: build(:user, organization: opinion.organization))
    end
  end
  ...
  context "when executing" do
    ...
    it "updates metric records" do
      create(:metric, metric_type: "endorsements", day: day, cumulative: 1, quantity: 1, organization: organization, category: nil, participatory_space: participatory_space, related_object: opinion)
      registry = generate_metric_registry
