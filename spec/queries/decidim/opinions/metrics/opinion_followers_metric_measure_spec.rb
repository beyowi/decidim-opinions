...
describe Decidim::Opinions::Metrics::OpinionFollowersMetricMeasure do
  ...
  let(:draft) { create(:collaborative_draft, published_at: day, component: opinions_component) }
  let!(:follows) do
    create_list(:follow, 10, followable: opinion, created_at: day)
    create_list(:follow, 10, followable: draft, created_at: day)
    create_list(:follow, 10, followable: opinion, created_at: day - 1.week)
  end
