    ...
    module Admin
      describe OpinionsMergeForm do
        ...
        context "when not enough opinions" do
          let(:opinions) { create_list(:opinion, 1, component: component) }
        ...
        context "when merging to the same component" do
          ...
          context "when a opinion has an endorsement" do
            before do
              create(:endorsement, resource: opinions.sample, author: create(:user, organization: component.participatory_space.organization))
            end
