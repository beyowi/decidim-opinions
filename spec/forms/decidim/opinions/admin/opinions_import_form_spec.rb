...
module Decidim
  module Opinions
    module Admin
      describe OpinionsImportForm do
        subject { form }

        let(:opinion) { create(:opinion) }
        let(:component) { opinion.component }
        let(:origin_component) { create(:opinion_component, participatory_space: component.participatory_space) }
        let(:states) { %w(accepted) }
        let(:import_opinions) { true }
        let(:params) do
          {
            states: states,
            keep_authors: false,
            origin_component_id: origin_component.try(:id),
            import_opinions: import_opinions
          }
        end
        ...
        context "when the import opinions is not accepted" do
          let(:import_opinions) { false }
        ...
        describe "origin_component" do
          let(:origin_component) { create(:opinion_component) }
