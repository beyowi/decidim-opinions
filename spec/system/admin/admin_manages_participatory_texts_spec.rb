...
describe "Admin manages participatory texts", type: :system do
  ...
  def import_document
    ...
    expect(page).to have_content "The following sections have been converted to opinions. Now you can review and adjust them before publishing."
    expect(page).to have_content "Preview participatory text"
  end
  ...
  def publish_participatory_text
    ...
    expect(page).to have_content "All opinions have been published"
  end

  def validate_published
    opinions = Decidim::Opinions::Opinion.where(component: current_component)
    titles = [
    ...
    expect(opinions.count).to eq(titles.size)
    expect(opinions.published.count).to eq(titles.size)
    expect(opinions.published.order(:position).pluck(:title)).to eq(titles)
  end

  def edit_participatory_text_body(index, new_body)
    fill_in(
      "preview_participatory_text_opinions_attributes_#{index}_body",
      with: new_body
    )
  end
  ...
  describe "accessing participatory texts in draft mode" do
    let!(:opinion) { create :opinion, :draft, component: current_component, participatory_text_level: "section" }

    it "renders only draft opinions" do
      visit_participatory_texts
  ...
  describe "discarding participatory texts in draft mode" do
    let!(:opinions) { create_list(:opinion, 5, :draft, component: current_component, participatory_text_level: "article") }

    it "removes all opinions in draft mode" do
      visit_participatory_texts
  ...
  describe "updating participatory texts in draft mode" do
    let!(:opinion) { create :opinion, :draft, component: current_component, participatory_text_level: "article" }
    let!(:new_body) { Faker::Lorem.unique.sentences(3).join("\n") }

    it "persists changes and all opinions remain as drafts" do
      visit_participatory_texts
      ...
      validate_occurrences(sections: 0, subsections: 0, articles: 1)
      opinion.reload
      expect(opinion.body.delete("\r")).to eq(new_body)
    end
  end
