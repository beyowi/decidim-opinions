...
describe "Explore versions", versioning: true, type: :system do
  ...
  let!(:emendation) { create(:opinion, body: "Amended One liner body", component: component) }
  let!(:amendment) { create :amendment, amendable: opinion, emendation: emendation }

  let(:form) do
    Decidim::Amendable::ReviewForm.from_params(
      id: amendment.id,
      amendable_gid: opinion.to_sgid.to_s,
  ...
  context "when visiting a opinion details" do
    before do
      visit opinion_path
    end
  ...
  context "when visiting versions index" do
    ...
    it "allows going back to the opinion" do
      click_link "Go back to opinion"
      expect(page).to have_current_path opinion_path
    end
  ...
  context "when showing version" do
    ...
    it "allows going back to the versions list" do
      ...
      expect(page).to have_current_path opinion_path + "/versions"
    end
