...
shared_examples "a opinion form" do |options|
  subject { form }
  ...
  context "when geocoding is enabled" do
    let(:component) { create(:opinion_component, :with_geocoding_enabled, participatory_space: participatory_space) }
  ...
  it "properly maps category id from model" do
    opinion = create(:opinion, component: component, category: category)

    expect(described_class.from_model(opinion).category_id).to eq(category_id)
  end

  if options && options[:user_group_check]
    it "properly maps user group id from model" do
      opinion = create(:opinion, component: component, users: [author], user_groups: [user_group])

      expect(described_class.from_model(opinion).user_group_id).to eq(user_group_id)
    end
  end
...
shared_examples "a opinion form with meeting as author" do |options|
  subject { form }
