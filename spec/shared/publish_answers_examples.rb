...
shared_examples "publish answers" do
  context "when publishing answers at once" do
    before do
      create_list :opinion, 3, :accepted_not_published, component: current_component
    ...
    it "publishes some answers" do
      ...
      page.first("[data-published-state=false] .js-opinion-list-check").set(false)

      click_button "Actions"
      click_button "Publish answers"

      within "#js-publish-answers-actions" do
        expect(page).to have_content("Answers for 2 opinions will be published.")
      end
    ...
    it "can't publish answers for non answered opinions" do
      page.find("#opinions_bulk.js-check-all").set(true)
      page.all("[data-published-state=false] .js-opinion-list-check").each { |c| c.set(false); }
