...
shared_examples "when managing opinions category as an admin" do
  ...
  let!(:opinion_first) { reportables.first }
  let!(:opinion_last) { reportables.last }
    ...
    context "when selecting opinions" do
      ...
      context "when submiting form" do
        before do
          click_button "Actions"
          click_button "Change category"
          within "#js-form-recategorize-opinions" do
