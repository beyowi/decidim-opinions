...
module Decidim
  module Opinions
    module Metrics
      class OpinionsMetricManage < Decidim::MetricManage
        def metric_name
          "opinions"
        end
        ...
        def query
          ...
          @query = @query.where("decidim_opinions_opinions.published_at <= ?", end_time).except_withdrawn.not_hidden
        ...
        def quantity
          @quantity ||= query.where("decidim_opinions_opinions.published_at >= ?", start_time).count
        end
      end
    end
  end
