...
module Decidim
  module Opinions
    ...
    class Opinion < Opinions::ApplicationRecord
      ...
      include Decidim::Opinions::CommentableOpinion
      ...
      include Decidim::Opinions::ParticipatoryTextSection
      include Decidim::Amendable
      include Decidim::NewsletterParticipant
      include Decidim::Randomable
      include Decidim::Endorsable
      include Decidim::Opinions::Valuatable
      ...
      amendable(
        fields: [:title, :body],
        form: "Decidim::Opinions::OpinionForm"
      )

      component_manifest_name "opinions"

      has_many :votes,
               -> { final },
               foreign_key: "decidim_opinion_id",
               class_name: "Decidim::Opinions::OpinionVote",
               dependent: :destroy,
               counter_cache: "opinion_votes_count"

      has_many :notes, foreign_key: "decidim_opinion_id", class_name: "OpinionNote", dependent: :destroy, counter_cache: "opinion_notes_count"

      validates :title, :body, presence: true

      geocoded_by :address, http_headers: ->(opinion) { { "Referer" => opinion.component.organization.host } }
      ...
      def self.with_valuation_assigned_to(user, space)
        ...
        includes(:valuation_assignments)
          .where(decidim_opinions_valuation_assignments: { valuator_role_id: valuator_roles })
      end
      ...
      searchable_fields({
                        ...
                        index_on_create: ->(opinion) { opinion.official? },
                        index_on_update: ->(opinion) { opinion.visible? })

      def self.log_presenter_class_for(_log)
        Decidim::Opinions::AdminLog::OpinionPresenter
      end
      ...
      def self.retrieve_opinions_for(component)
        Decidim::Opinions::Opinion.where(component: component).joins(:coauthorships)
      ...
      def self.newsletter_participant_ids(component)
        opinions = retrieve_opinions_for(component).uniq

        coauthors_recipients_ids = opinions.map { |p| p.notifiable_identities.pluck(:id) }.flatten.compact.uniq

        participants_has_voted_ids = Decidim::Opinions::OpinionVote.joins(:opinion).where(opinion: opinions).joins(:author).map(&:decidim_author_id).flatten.compact.uniq

        endorsements_participants_ids = Decidim::Endorsement.where(resource: opinions)
                                                            .where(decidim_author_type: "Decidim::UserBaseEntity")
                                                            .map(&:decidim_author_id).flatten.compact.uniq

        commentators_ids = Decidim::Comments::Comment.user_commentators_ids_in(opinions)

        (endorsements_participants_ids + participants_has_voted_ids + coauthors_recipients_ids + commentators_ids).flatten.compact.uniq
      end

      # Public: Updates the vote count of this opinion.
      #
      # Returns nothing.
      # rubocop:disable Rails/SkipsModelValidations
      def update_votes_count
        update_columns(opinion_votes_count: votes.count)
      end
      # rubocop:enable Rails/SkipsModelValidations

      # Public: Check if the user has voted the opinion.
      #
      # Returns Boolean.
      def voted_by?(user)
        OpinionVote.where(opinion: self, author: user).any?
      end

      # Public: Checks if the opinion has been published or not.
      #
      # Returns Boolean.
      def published?
      ...
      # Public: Returns the published state of the opinion.
      #
      # Returns Boolean.
      def state
      ...
      # Public: Returns the internal state of the opinion.
      #
      # Returns Boolean.
      def internal_state
        return amendment.state if emendation?
      ...
      # Public: Checks if the organization has published the state for the opinion.
      #
      # Returns Boolean.
      def published_state?
      ...
      # Public: Checks if the organization has given an answer for the opinion.
      #
      # Returns Boolean.
      def answered?
        answered_at.present?
      ...
      # Public: Checks if the author has withdrawn the opinion.
      #
      # Returns Boolean.
      def withdrawn?
      ...
      # Public: Checks if the organization has accepted a opinion.
      #
      # Returns Boolean.
      def accepted?
        state == "accepted"
      ...
      # Public: Checks if the organization has rejected a opinion.
      #
      # Returns Boolean.
      def rejected?
        state == "rejected"
      ...
      # Public: Checks if the organization has marked the opinion as evaluating it.
      #
      # Returns Boolean.
      def evaluating?
        state == "evaluating"
      ...
      # Public: Whether the opinion is official or not.
      def official?
        authors.first.is_a?(Decidim::Organization)
      end

      # Public: Whether the opinion is created in a meeting or not.
      def official_meeting?
        authors.first.class.name == "Decidim::Meetings::Meeting"
      end

      # Public: The maximum amount of votes allowed for this opinion.
      #
      # Returns an Integer with the maximum amount of votes, nil otherwise.
      def maximum_votes
        maximum_votes = component.settings.threshold_per_opinion
        return nil if maximum_votes.zero?

        maximum_votes
      end

      # Public: The maximum amount of votes allowed for this opinion. 0 means infinite.
      #
      ...
      # Public: Can accumulate more votres than maximum for this opinion.
      #
      ...
      # Checks whether the user can edit the given opinion.
      #
      ...
      # Checks whether the user can withdraw the given opinion.
      #
      ...
      # Public: Whether the opinion is a draft or not.
      def draft?
        published_at.nil?
      end
      ...
      ransacker :commentable_comments_count do
        ...
        (SELECT COUNT(decidim_comments_comments.id)
         ...
         WHERE decidim_comments_comments.decidim_commentable_id = decidim_opinions_opinions.id
         AND decidim_comments_comments.decidim_commentable_type = 'Decidim::Opinions::Opinion'
      ...
      def self.sort_by_valuation_assignments_count_nulls_last_query
        <<-SQL
        (
          SELECT COUNT(decidim_opinions_valuation_assignments.id)
          FROM decidim_opinions_valuation_assignments
          WHERE decidim_opinions_valuation_assignments.decidim_opinion_id = decidim_opinions_opinions.id
          GROUP BY decidim_opinions_valuation_assignments.decidim_opinion_id
        )
        SQL
      end
      ...
      def self.valuator_role_ids_has(value)
        query = <<-SQL
        :value = any(
          (SELECT decidim_opinions_valuation_assignments.valuator_role_id
          FROM decidim_opinions_valuation_assignments
          WHERE decidim_opinions_valuation_assignments.decidim_opinion_id = decidim_opinions_opinions.id
          )
        )
      ...
      ransacker :state_published do
        Arel.sql("CASE
          WHEN EXISTS (
            ...
            WHERE decidim_amendments.decidim_emendation_type = 'Decidim::Opinions::Opinion'
            AND decidim_amendments.decidim_emendation_id = decidim_opinions_opinions.id
          ) THEN 0
      ...
      ransacker :id_string do
        Arel.sql(%{cast("decidim_opinions_opinions"."id" as text)})
      end
      ...
      def self.export_serializer
        Decidim::Opinions::OpinionSerializer
      end
      ...
      # Checks whether the opinion is inside the time window to be editable or not once published.
      def within_edit_time_limit?
        return true if draft?

        limit = updated_at + component.settings.opinion_edit_before_minutes.minutes
        Time.current < limit
      end
      ...
      def copied_from_other_component?
        linked_resources(:opinions, "copied_from_component").any?
      end
    end
  end
end
