...
module Decidim
  module Opinions
    module Admin
      ...
      class OpinionsImportForm < Decidim::Form
        mimic :opinions_import

        attribute :origin_component_id, Integer
        attribute :import_opinions, Boolean
        ...
        validates :import_opinions, allow_nil: false, acceptance: true
        ...
        def states_collection
          VALID_STATES.map do |state|
            OpenStruct.new(
              name: I18n.t(state, scope: "decidim.opinions.answers"),
              value: state
            )
          end
        end
        ...
        def origin_components
          @origin_components ||= current_participatory_space.components.where.not(id: current_component.id).where(manifest_name: :opinions)
        end
