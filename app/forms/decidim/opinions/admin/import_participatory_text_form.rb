...
module Decidim
  module Opinions
    module Admin
      # A form object to be used when admin users want to import a collection of opinions
      # from a participatory text.
      class ImportParticipatoryTextForm < Decidim::Form
        include TranslatableAttributes

        ACCEPTED_MIME_TYPES = Decidim::Opinions::DocToMarkdown::ACCEPTED_MIME_TYPES
        ...
        # Assume it's a NEW participatory_text if there are no opinions
        # Validate document presence while CREATING opinions from document
        # Allow skipping document validation while UPDATING title/description
        def new_participatory_text?
          Decidim::Opinions::Opinion.where(component: current_component).blank?
        end
        ...
        def i18n_valid_mime_types_text
          ACCEPTED_MIME_TYPES.keys.map do |mime_type|
            I18n.t(mime_type, scope: "decidim.opinions.admin.participatory_texts.new_import.accepted_mime_types")
          end.join(", ")
        end
