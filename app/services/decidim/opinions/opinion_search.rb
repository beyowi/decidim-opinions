...
module Decidim
  module Opinions
    ...
    # opinions in a participatory process.
    class OpinionSearch < ResourceSearch
      # Public: Initializes the service.
      # component     - A Decidim::Component to get the opinions from.
      # page        - The page number to paginate the results.
      # per_page    - The number of opinions to return per page.
      def initialize(options = {})
        @component = options[:component]
        @current_user = options[:current_user]

        base = options[:state]&.member?("withdrawn") ? Opinion.withdrawn : Opinion.except_withdrawn
        super(base, options)
      end
      ...
      # Filters Opinions by the name of the classes they are linked to. By default,
      # returns all Opinions. When a `related_to` param is given, then it camelcases item
      # to find the real class name and checks the links for the Opinions.
      #
      ...
      #
      # Returns only those opinions that are linked to the given class name.
