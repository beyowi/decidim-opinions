...
module Decidim
  module Opinions
    class DiffRenderer < BaseDiffRenderer
      ...
      # Sets the previous value so the emendation can be compared with the amended opinion.
      # If the amendment is being evaluated, returns the CURRENT attribute value of the amended opinion;
      # else, returns the attribute value of amended opinion at the moment of making the amendment.
      def amended_previous_value(attribute)
        if opinion.amendment.evaluating?
          opinion.amendable.attributes[attribute.to_s]
        else # See Decidim::Amendable::PublishDraft#set_first_emendation_version
          opinion.versions.first.changeset[attribute.to_s].last
        end
      end
      ...
      def opinion
        @opinion ||= Opinion.find_by(id: version.item_id)
      end
    end
  end
end
