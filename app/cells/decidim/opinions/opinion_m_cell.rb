...
module Decidim
  module Opinions
    # This cell renders a opinion with its M-size card.
    class OpinionMCell < Decidim::CardMCell
      ...
      def badge_classes
        ...
        state_classes.concat(["label", "opinion-status"]).join(" ")
      end
      ...
      def progress_bar_subtitle_text
        if progress_bar_progress >= progress_bar_total
          t("decidim.opinions.opinions.votes_count.most_popular_opinion")
        else
          t("decidim.opinions.opinions.votes_count.need_more_votes")
        end
      end
