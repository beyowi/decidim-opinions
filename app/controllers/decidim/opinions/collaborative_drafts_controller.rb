...
module Decidim
  module Opinions
    ...
    class CollaborativeDraftsController < Decidim::Opinions::ApplicationController
      ...
      def compare
        @step = :step_2
        @similar_collaborative_drafts ||= Decidim::Opinions::SimilarCollaborativeDrafts
        ...
        if @similar_collaborative_drafts.blank?
          flash[:notice] = I18n.t("opinions.collaborative_drafts.compare.no_similars_found", scope: "decidim")
      ...
      def create
        ...
        CreateCollaborativeDraft.call(@form, current_user) do
          on(:ok) do |collaborative_draft|
            flash[:notice] = I18n.t("opinions.collaborative_drafts.create.success", scope: "decidim")

            redirect_to Decidim::ResourceLocatorPresenter.new(collaborative_draft).path
          end

          on(:invalid) do
            flash.now[:alert] = I18n.t("opinions.collaborative_drafts.create.error", scope: "decidim")
            render :complete
          end
        end
      end
      ...
      def update
        ...
        UpdateCollaborativeDraft.call(@form, current_user, @collaborative_draft) do
          on(:ok) do |collaborative_draft|
            flash[:notice] = I18n.t("opinions.collaborative_drafts.update.success", scope: "decidim")
            redirect_to Decidim::ResourceLocatorPresenter.new(collaborative_draft).path
          end

          on(:invalid) do
            flash.now[:alert] = I18n.t("opinions.collaborative_drafts.update.error", scope: "decidim")
            render :edit
          end
        end
      end
      ...
      def withdraw
        WithdrawCollaborativeDraft.call(@collaborative_draft, current_user) do
          on(:ok) do
            flash[:notice] = t("withdraw.success", scope: "decidim.opinions.collaborative_drafts.collaborative_draft")
          end

          on(:invalid) do
            flash.now[:alert] = t("withdraw.error", scope: "decidim.opinions.collaborative_drafts.collaborative_draft")
          end
        end
        redirect_to Decidim::ResourceLocatorPresenter.new(@collaborative_draft).path
      end
      ...
      def publish
        PublishCollaborativeDraft.call(@collaborative_draft, current_user) do
          on(:ok) do |opinion|
            flash[:notice] = I18n.t("publish.success", scope: "decidim.opinions.collaborative_drafts.collaborative_draft")
            redirect_to Decidim::ResourceLocatorPresenter.new(opinion).path
          end

          on(:invalid) do
            flash.now[:alert] = t("publish.error", scope: "decidim.opinions.collaborative_drafts.collaborative_draft")
            redirect_to Decidim::ResourceLocatorPresenter.new(@collaborative_draft).path
          end
        end
      end
      ...
      def form_presenter
        @form_presenter ||= present(@form, presenter_class: Decidim::Opinions::CollaborativeDraftPresenter)
      end
