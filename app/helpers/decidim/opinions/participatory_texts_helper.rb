...
module Decidim
  module Opinions
    ...
    module ParticipatoryTextsHelper
      ...
      #
      # opinion - The current opinion item.
      #
      # Returns a string with the title of the section, subsection or article.
      def preview_participatory_text_section_title(opinion)
        title = decidim_html_escape(present(opinion).title)
        translated = t(opinion.participatory_text_level, scope: "decidim.opinions.admin.participatory_texts.sections", title: title)
        translated.html_safe
      end

      def render_participatory_text_title(participatory_text)
        if participatory_text.nil?
          t("alternative_title", scope: "decidim.opinions.participatory_text_opinion")
        else
