...
module Decidim
  module Opinions
    # Simple helpers to handle markup variations for opinion wizard partials
    module OpinionWizardHelper
      # Returns the css classes used for the opinion wizard for the desired step
      #
      ...
      def opinion_wizard_step_classes(step, current_step)
        step_i = step.to_s.split("_").last.to_i
        if step_i == opinion_wizard_step_number(current_step)
          %(step--active #{step} #{current_step})
        elsif step_i < opinion_wizard_step_number(current_step)
          %(step--past #{step})
        else
          %()
        end
      ...
      #
      # step - A symbol of the target step
      def opinion_wizard_step_name(step)
      ...
      def opinion_wizard_step_title(action_name)
        ...
        t("decidim.opinions.#{type_of}.#{step_title}.title")
      end
      ...
      def opinion_wizard_steps_title
        t("title", scope: "decidim.opinions.#{type_of}.wizard_steps")
      end
      ...
      def wizard_aside_info_text
        case type_of
        when :collaborative_drafts
          t("info", scope: "decidim.opinions.collaborative_drafts.wizard_aside").html_safe
        else
          t("info", scope: "decidim.opinions.opinions.wizard_aside").html_safe
        end
      end
      ...
      def wizard_aside_back_text(from = nil)
        ...
        case type_of
        when :collaborative_drafts
          t(key, scope: "decidim.opinions.collaborative_drafts.wizard_aside").html_safe
        else
          t(key, scope: "decidim.opinions.opinions.wizard_aside").html_safe
        end
      end

      def type_of
        if ["Decidim::Opinions::CollaborativeDraftForm"].include? @form.class.name
