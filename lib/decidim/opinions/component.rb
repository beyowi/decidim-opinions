...
Decidim.register_component(:opinions) do |component|
  component.engine = Decidim::Opinions::Engine
  component.admin_engine = Decidim::Opinions::AdminEngine
  component.icon = "decidim/opinions/icon.svg"

  component.on(:before_destroy) do |instance|
    raise "Can't destroy this component when there are opinions" if Decidim::Opinions::Opinion.where(component: instance).any?
  end

  component.data_portable_entities = ["Decidim::Opinions::Opinion"]

  component.newsletter_participant_entities = ["Decidim::Opinions::Opinion"]

  component.actions = %w(endorse vote create withdraw amend)

  component.query_type = "Decidim::Opinions::OpinionsType"

  component.permissions_class_name = "Decidim::Opinions::Permissions"

  component.settings(:global) do |settings|
    settings.attribute :vote_limit, type: :integer, default: 0
    settings.attribute :minimum_votes_per_user, type: :integer, default: 0
    settings.attribute :opinion_limit, type: :integer, default: 0
    settings.attribute :opinion_length, type: :integer, default: 500
    settings.attribute :opinion_edit_before_minutes, type: :integer, default: 5
    settings.attribute :threshold_per_opinion, type: :integer, default: 0
    settings.attribute :can_accumulate_supports_beyond_threshold, type: :boolean, default: false
    settings.attribute :opinion_answering_enabled, type: :boolean, default: true
    settings.attribute :official_opinions_enabled, type: :boolean, default: true
    ...
    settings.attribute :participatory_texts_enabled,
                       ...
                       readonly: ->(context) { Decidim::Opinions::Opinion.where(component: context[:component]).any? }
    ...
    settings.attribute :new_opinion_body_template, type: :text, translated: true, editor: false, required: false
    settings.attribute :new_opinion_help_text, type: :text, translated: true, editor: true
    settings.attribute :opinion_wizard_step_1_help_text, type: :text, translated: true, editor: true
    settings.attribute :opinion_wizard_step_2_help_text, type: :text, translated: true, editor: true
    settings.attribute :opinion_wizard_step_3_help_text, type: :text, translated: true, editor: true
    settings.attribute :opinion_wizard_step_4_help_text, type: :text, translated: true, editor: true
  end
  ...
  component.register_resource(:opinion) do |resource|
    resource.model_class_name = "Decidim::Opinions::Opinion"
    resource.template = "decidim/opinions/opinions/linked_opinions"
    resource.card = "decidim/opinions/opinion"
    resource.actions = %w(endorse vote amend)
    resource.searchable = true
  end

  component.register_resource(:collaborative_draft) do |resource|
    resource.model_class_name = "Decidim::Opinions::CollaborativeDraft"
    resource.card = "decidim/opinions/collaborative_draft"
  end

  component.register_stat :opinions_count, primary: true, priority: Decidim::StatsRegistry::HIGH_PRIORITY do |components, start_at, end_at|
    Decidim::Opinions::FilteredOpinions.for(components, start_at, end_at).published.except_withdrawn.not_hidden.count
  end

  component.register_stat :opinions_accepted, primary: true, priority: Decidim::StatsRegistry::HIGH_PRIORITY do |components, start_at, end_at|
    Decidim::Opinions::FilteredOpinions.for(components, start_at, end_at).accepted.not_hidden.count
  end

  component.register_stat :supports_count, priority: Decidim::StatsRegistry::HIGH_PRIORITY do |components, start_at, end_at|
    opinions = Decidim::Opinions::FilteredOpinions.for(components, start_at, end_at).published.not_hidden
    Decidim::Opinions::OpinionVote.where(opinion: opinions).count
  end

  component.register_stat :endorsements_count, priority: Decidim::StatsRegistry::MEDIUM_PRIORITY do |components, start_at, end_at|
    opinions = Decidim::Opinions::FilteredOpinions.for(components, start_at, end_at).not_hidden
    Decidim::Endorsement.where(resource_id: opinions.pluck(:id), resource_type: Decidim::Opinions::Opinion.name).count
  end

  component.register_stat :comments_count, tag: :comments do |components, start_at, end_at|
    opinions = Decidim::Opinions::FilteredOpinions.for(components, start_at, end_at).published.not_hidden
    Decidim::Comments::Comment.where(root_commentable: opinions).count
  end
  ...
  component.register_stat :followers_count, tag: :followers, priority: Decidim::StatsRegistry::LOW_PRIORITY do |components, start_at, end_at|
    opinions_ids = Decidim::Opinions::FilteredOpinions.for(components, start_at, end_at).published.not_hidden.pluck(:id)
    Decidim::Follow.where(decidim_followable_type: "Decidim::Opinions::Opinion", decidim_followable_id: opinions_ids).count
  end

  component.exports :opinions do |exports|
    exports.collection do |component_instance, user|
      space = component_instance.participatory_space

      collection = Decidim::Opinions::Opinion
                   .published
    ...
    exports.serializer Decidim::Opinions::OpinionSerializer
  end

  component.exports :comments do |exports|
    exports.collection do |component_instance|
      Decidim::Comments::Export.comments_for_resource(
        Decidim::Opinions::Opinion, component_instance
      )
    end
  ...
  component.seeds do |participatory_space|
    ...
    5.times do |n|
      ...
      opinion = Decidim.traceability.perform_action!(
        ...
        opinion = Decidim::Opinions::Opinion.new(params)
        opinion.add_coauthor(participatory_space.organization)
        opinion.save!
        opinion
      end
      ...
      if n.positive?
        Decidim::User.where(decidim_organization_id: participatory_space.decidim_organization_id).all.sample(n).each do |author|
          ...
          opinion.add_coauthor(author, user_group: user_group)
        end
      end

      if opinion.state.nil?
        ...
        params = {
          ...
          category: participatory_space.categories.sample,
          scope: Faker::Boolean.boolean(0.5) ? global : scopes.sample,
          title: "#{opinion.title} #{Faker::Lorem.sentence(1)}",
          body: "#{opinion.body} #{Faker::Lorem.sentence(3)}",
          state: "evaluating",
          answer: nil,
        ...
        emendation = Decidim.traceability.perform_action!(
          ...
          emendation = Decidim::Opinions::Opinion.new(params)
      ...
      (n % 3).times do |m|
        ...
        Decidim::Opinions::OpinionVote.create!(opinion: opinion, author: author) unless opinion.published_state? && opinion.rejected?
        Decidim::Opinions::OpinionVote.create!(opinion: emendation, author: author) if emendation
      end

      unless opinion.published_state? && opinion.rejected?
        (n * 2).times do |index|
          ...
          Decidim::Endorsement.create!(resource: opinion, author: author, user_group: author.user_groups.first)
        end
      end
      ...
      (n % 3).times do
        ...
        Decidim::Opinions::OpinionNote.create!(
          opinion: opinion,
          author: author_admin,
          body: Faker::Lorem.paragraphs(2).join("\n")
        )
      ...
      Decidim::Comments::Seed.comments_for(opinion)
      ...
      draft = Decidim.traceability.perform_action!("create", Decidim::Opinions::CollaborativeDraft, author) do
        draft = Decidim::Opinions::CollaborativeDraft.new(
          component: component,
          category: participatory_space.categories.sample,
          scope: Faker::Boolean.boolean(0.5) ? global : scopes.sample,
          title: Faker::Lorem.sentence(2),
    ...
    Decidim.traceability.update!(
      Decidim::Opinions::CollaborativeDraft.all.sample,
