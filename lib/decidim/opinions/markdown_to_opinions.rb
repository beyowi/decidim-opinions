...
module Decidim
  module Opinions
    ...
    # produces Opinions in the form of sections and articles.
    #
    # This implementation uses Redcarpet Base renderer.
    # Redcarpet::Render::Base performs a callback for every block it finds, what MarkdownToOpinions
    # does is to implement callbacks for the blocks which it is interested in performing some actions.
    #
    class MarkdownToOpinions < ::Redcarpet::Render::Base
      ...
      # Creates Paricipatory Text Opinions at Section and Subsection levels.
      ...
      # Creates Paricipatory Text Opinions at Article level.
      def paragraph(text)
      ...
      # Prevents PaperTrail from creating versions while producing opinions from a document.
      # A first version will be created when publishing the Participatory Text.
      def create_opinion(title, body, participatory_text_level)
        attributes = {
          component: @component,
          title: title,
          body: body,
        ...
        PaperTrail.request(enabled: false) do
          opinion = Decidim::Opinions::OpinionBuilder.create(
            attributes: attributes,
            author: @component.organization,
            action_user: @current_user
          )

          @last_position = opinion.position
